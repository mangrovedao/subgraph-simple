...
export function handleOfferSuccess(event: OfferSuccess): void {
  ...
  const volume = getOrCreateAccountVolumeByPair(offer.owner !== null ? offer.owner! : offer.maker, event.params.outbound_tkn, event.params.inbound_tkn, event.block.timestamp, currentOrderIsSnipe() ? "MakerSniped" : "Maker");
